...
<div className='flex items-center gap-2'>
  ...
  {typeof data.image ==='object'&&(
    <div className='flex flex-col gap-1 pl-4 text-sm'>
     ...
     <span className='dot absolute left-1 top-1 w-3 h-3 bg-rounded-full transition-transform duration-200 ease-in-out peer-checked:translate-x-4  '></span>
        </label>
    </div>
  )}
</div>
...
<Icon className="size-4"/>
     ...
     {field.required && <span className="text-red-500">*</span>}
      ...
      <input type={field.type} value={data[field.key] || ""}onChange={(e)=>handelChange(field.key , e.target.value)} className='mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring focus-ring-blue-500 focus:border-blue-500 outline-none transition-color text-sm' placeholder={`Enter your ${field.label.toLowerCase()}`} required={field.required}/> 
